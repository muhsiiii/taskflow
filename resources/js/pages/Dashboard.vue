<template>
  <AppLayout>
    ...
    <div class="flex justify-between items-center mb-8">
      <div>
        <h1 class="text-2xl font-bold" style="color:#f1f5f9;">Dashboard
    ...
    <div class="grid grid-cols-4 gap-4 mb-8">
      <div v-for="stat in stats" :key="stat.label" class="rounded-2xl p-5 relative overflow-hidden"
    ...
    <div class="flex justify-between items-center mb-5">
      <div class="flex items-center gap-3">
        ...
        <div v-if="loading" class="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin">
        </div>
      </div>
    ...
    <div class="grid grid-cols-3 gap-5">
      <div v-for="col in columns" :key="col.status" class="rounded-2xl p-4 min-h-96 transition-all"
        :style="`background:#1a1a2e;border:2px solid ${dragOver === col.status ? col.color : '#2d2d4e'};`"
        @dragover.prevent="dragOver = col.status" @dragleave="dragOver = null" @drop="onDrop($event, col.status)">
        ...
        <div class="space-y-3">
          <div v-for="task in tasksByStatus[col.status]" :key="task.id"
            class="rounded-xl p-4 cursor-grab transition-all hover:-translate-y-0.5"
            style="background:#0f0f1a;border:1px solid #2d2d4e;" draggable="true" @dragstart="dragTask = task.id"
            @dragend="dragOver = null">
            <div class="flex justify-between items-center mb-3">
              <span class="text-xs px-2 py-0.5 rounded-md font-medium" :style="priorityStyle(task.priority)">
                {{ task.priority.toUpperCase() }}
              </span>
              <div v-if="task.assignee" class="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold"
                style="background:linear-gradient(135deg,#6366f1,#8b5cf6);color:white;">
                {{ task.assignee.name.charAt(0).toUpperCase() }}
              </div>
            </div>
            <p class="text-sm font-medium mb-3" style="color:#e2e8f0;">{{ task.title }}</p>
            <p v-if="task.description" class="text-xs mb-3 line-clamp-2" style="color:#4a5568;">{{ task.description }}
            </p>
            <div class="flex justify-between items-center pt-2" style="border-top:1px solid #2d2d4e;">
              <span v-if="task.due_date" class="text-xs" :style="isOverdue(task) ? 'color:#ef4444;' : 'color:#4a5568;'">
                📅 {{ task.due_date }}
              </span>
              <span v-else class="text-xs" style="color:#2d2d4e;">No date</span>
              <div class="flex gap-2">
                <span v-if="task.subtasks_count" class="text-xs" style="color:#4a5568;">🔗 {{ task.subtasks_count
                  }}</span>
                <span v-if="task.attachments_count" class="text-xs" style="color:#4a5568;">📎 {{ task.attachments_count
                  }}</span>
              </div>
            </div>
          </div>
    ...
    <Transition name="fade">
      <div v-if="showModal" class="fixed inset-0 flex items-center justify-center z-50 p-4"
        ...
        <div class="w-full max-w-md rounded-2xl p-6 shadow-2xl" style="background:#1a1a2e;border:1px solid #2d2d4e;">
          <div class="flex justify-between items-center mb-5">
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-xs mb-1.5 font-medium uppercase tracking-wide" style="color:#64748b;">TITLE
                *</label>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs mb-1.5 font-medium uppercase tracking-wide"
                  style="color:#64748b;">PRIORITY</label>
                <select v-model="form.priority" class="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                  style="background:#0f0f1a;border:1px solid #2d2d4e;color:#e2e8f0;">
...
const stats = computed(() => [
  ...
  { label: 'Completed', value: doneTasks.value.length, color: '#22c55e', sub: `${completionPct.value}% done` },
...
async function createTask() {
  ...
  creating.value = true
  try {
    const res = await axios.post(`/api/workspaces/${WS}/projects/${PR}/tasks`, form.value, { headers: getHeaders() })
